<template>
    <div class="input_auto" @click.stop="focus">
        <span>
            <label v-for="log in selected" @click="remove">
                <strong>{{ log.rev_v }}</strong> {{ log.message }}
                <input type="hidden" :name="name+'[]'" :rev="log.rev" :value="JSON.stringify(log)">
            </label>
        </span>
        <input type="text" class="input_text" @input="find" @keydown="keyControl" @blur="hide">
        <ul class="lists">
            <li 
                v-for="(log, idx) in searched" 
                :class="{select: (idx==0) ? true : false}"
				@click.stop.prevent="revClick"
            >
			<revision>
				<strong>{{ log.rev_v }}</strong> {{ log.message }}
			</revision>
	...
	methods: {
		...
		revSelect(e) {
			...
			const exists = (this.selected.some((s) => {
				if (s.rev == this.searched[window.search_cursor].rev) return true;
			}));
			if (!exists) {
		...
		remove(e) {
			const rev = e.currentTarget.querySelector('input').getAttribute('rev');
			this.selected.forEach((log, idx) => {
				if (log.rev == rev) {
